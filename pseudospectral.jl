...
function PseudoSpectral(
    ...
    #sol = solve(prob, solver, dt = Δt, adaptive = false, reltol = 1e-8, abstol = 1e-8)
    sol = solve(prob, solver, dt = Δt, adaptive = true, reltol = 1e-8, abstol = 1e-8)
    Fψ = sol(0:config.Δt:config.t_end)

    ψ = Matrix{ComplexF64}(undef, (config.Ngrids, config.Nsteps + 1))
    Vₓ = Matrix{ComplexF64}(undef, (config.Ngrids, config.Nsteps + 1))
    for t in 1:config.Nsteps + 1
        ψ[:, t] = ifft(Fψ[:, t])
        Vₓ[:, t] = abs2.(ψ[:, t])
    end
